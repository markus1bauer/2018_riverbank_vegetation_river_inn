...
# Show figure 3: vegetation height ####
# Markus Bauer
# 2022-02-14
...
sites <- read_csv("data_processed_sites.csv",
  ...
  select(
    ...
    barrier_distance, treatment, habitatType, herbHeight
  ) %>%
  subset(herbHeight > 0)
...
ggplot(pdata, aes(year, herbHeight, shape = year, ymin = conf.low, ymax = conf.high)) +
  ...
  geom_quasirandom(
    data = sites, aes(year, herbHeight),
...
### Save ###
ggsave("figure_vegHeight_800dpi_12x8.5cm.tiff",
