# Show Figure vegetation height ~ year:treatment ####
...
(sites <- select(sites, no, plotTemp, plot, block, conf.low, conf.high, year, barrierDist, treatment, habitatType, herbHeight))
...
pdata <- rename(pdata, herbHeight = predicted, treatment = x, year = group)
meandata <- filter(pdata, year == "Control")
pd <- position_dodge(.6)
ggplot(pdata, aes(year, herbHeight, shape = year, ymin = conf.low, ymax = conf.high))+
  facet_grid(~treatment) +
  geom_quasirandom(data = sites, aes(year, herbHeight), 
                   color = "grey70", dodge.width = .6, size = 0.7)+
  geom_hline(aes(yintercept = herbHeight), meandata, 
  ...
  scale_y_continuous(limits = c(0,300), breaks = seq(-100,300,50)) +
  scale_shape_manual(values = c(1,16,16)) +
  labs(x = "", y = expression(paste("Vegetation height [cm]")), shape = "") +
  guides(shape = F) +
  themeMB()
